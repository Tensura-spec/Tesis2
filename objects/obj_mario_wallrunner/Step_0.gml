...
//Otherwise, if moving in a clockwise motion
    ...
    if ((input_check(input2.action_1)) && ((input_check(input.right)) || (gamepad_axis_value(0, gp_axislh) > 0.5)))
        speed += 0.1;
...
//Otherwise, if running in a counter-clockwise motion
    ...
    if ((input_check(input2.action_1)) && ((input_check(input.left)) || (gamepad_axis_value(0, gp_axislh) < -.5)))
        speed += 0.1;
