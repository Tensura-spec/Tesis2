...
|| (global.powerup == cs_cape) { //If the player is not flying
	...
	if (input_check_released(input2.action_1))			
		run_cooldown = 2;
    ...
    if (input_check(input2.action_1) || (run_cooldown > 0)) { //If the 'Run' key is being held.
...
//Otherwise, if the player is flying.
    ...
    if (flyfix == 1) {
        ...
        //Set running speed
        if (input_check(input2.action_1))
            xspeedmax = 2.6;           
...
&& (enable_control == true) {
    ...
    if (input_check_released(input2.action_0))
    && (jumping == 1)
        jumping = 2;
...
&& (enable_control == 1) {
	...
	&& (groundpound == 0)
    && (swimming == false)
    && (input_check_pressed(input2.action_0))
...
&& (groundpound == 0) {
    ...
    if (squirrelpropel == 0) {
        ...
        if ((yspeed > 0) && (input_check_pressed(input2.action_0))) {
        ...
        else if ((yspeed > 0.5) && (input_check(input2.action_0)))
            yspeed = 0.5
    }
