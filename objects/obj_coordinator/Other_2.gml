...
#region LOAD
	...
	if (file_exists("settings.dat")) {
		...
		try
		{
			...
			#region KEYBOARD KEYS
				...
				global.key[input2.action_0] = real(file_text_read_string(file));
				file_text_readln(file);
		
				//Set saved 'Action 1' Key (Default: Control)
				global.key[input2.action_1] = real(file_text_read_string(file));
				file_text_readln(file);
		
				//Set saved 'Action 2' Key (Default: Space)
				global.key[input2.action_2] = real(file_text_read_string(file));
				file_text_readln(file);
			...
			#region CONTROLLER KEYS
				...
				global.button[input2.action_0] = real(file_text_read_string(file));
				file_text_readln(file);
		
				//Set saved 'Action 1' button (Default: X)
				global.button[input2.action_1] = real(file_text_read_string(file));
				file_text_readln(file);
		
				//Set saved 'Action 2' button (Default: B)
				global.button[input2.action_2] = real(file_text_read_string(file));
				file_text_readln(file);
	...
	if (loadDefaultSettings)
	{
		...
		#region KEYBOARD KEYS
			...
			//Action 1 (Shift)
			global.key[input2.action_0] = vk_shift;
    
			//Action 2 (Control)
			global.key[input2.action_1] = (os_type != os_macosx) ? vk_control : vk_lcommand;
    
			//Action 3 (Space)
			global.key[input2.action_2] = vk_space;
		...
		#region CONTROLLER KEYS
			...
			//Action 0 (A)
			global.button[input2.action_0] = gp_face1;
    
			//Action 1 (X)
			global.button[input2.action_1] = gp_face3;
    
			//Action 2 (B)
			global.button[input2.action_2] = gp_face2;
